...
const getData = async () => {
  const res = await fetch(`${HOST}post/fetch-all-post-by-category/LifeStyle`, {
  ...
  return (
    <div className="container mt-4">
      <Row>
        {/* LifeStyle Column */}
        <Col md={6}>
          <h5 className="border-start border-3 border-danger ps-2 mb-3">LifeStyle</h5>
          ...
          {posts.length === 0 ? (
            ...
            posts.map((post, index) => (
              <Card key={index} className="text-white mb-3">
                <Card.Img src={post.image} alt={post.title} className={index === 1 ? 'tarvels-image-layout' : ''} />
